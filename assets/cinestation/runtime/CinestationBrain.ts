import { _decorator, Component, Node, Camera, lerp, clamp01, Enum, primitives, utils, Mesh, Material, Vec2, Vec3, isValid } from 'cc';
import { cinestation } from './CinestationData';
import { cinestationShareAssets } from './CinestationShareAssets';
import { Nullable } from './Common/Types';
import { Visualization } from './Common/Visualization';
import { VirtualCamera } from './VirtualCamera';
const { ccclass, property, menu, executionOrder, executeInEditMode } = _decorator;
...
@menu('Cinestation/CinestationBrain')
@executeInEditMode
@executionOrder(-1)
export class CinestationBrain extends Visualization {
    ...
    private _getActiveCamera() {
        let vcam = cinestation.vcam = CC_EDITOR ? this.__selectedCamera : cinestation.getPriorCamera();
        this.visible = CC_EDITOR ? !!this.__selectedCamera : vcam && vcam.debug;
        this._setDebugProperties(vcam);
        return vcam;
    }
