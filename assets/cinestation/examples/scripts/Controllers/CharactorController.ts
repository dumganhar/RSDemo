...
import { _decorator, Component, Node, Animation, Vec3, Quat, lerp, Vec4, clamp01, game, director } from 'cc';
...
export class CharactorController extends Component {
    ...
    public setDirection(dir: Nullable<Vec3>) {
        if (dir) {
            Quat.fromViewUp(this._rotationTo, dir);
            this._rotateSpeedTo = 10;
        }
        else {
            this._rotateSpeed = this._rotateSpeedTo = 0;
        }
    ...
    public update(dt: number) {
        this._rotateSpeed = lerp(this._rotateSpeed, this._rotateSpeedTo, clamp01(10 * dt));
        this._movingSpeed = lerp(this._movingSpeed, this._movingSpeedTo, clamp01(5 * dt));

        if (this._rotateSpeed > 0) {
            this.node.rotation = this.node.rotation.lerp(this._rotationTo, this._rotateSpeed * dt);
        }
        if (this._movingSpeed > 0) {
            this.node.position = this.node.position.add(Vec3.multiplyScalar(__offset, this.node.forward.negative(), this._movingSpeed * dt));
        }
    }
}
